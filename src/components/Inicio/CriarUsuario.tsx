...
export default function CriarUsuario({ navigation }: Props) {
  ...
  const BotoesSelecionados = (valor: string) => {
    ...
    if (selected.includes(valor)) {
      setSelected(selected.filter((itens) => itens !== valor));
          ...
          {choices.map((choice) => (
            ...
            >
              <Image
                ...
                className={`${selected.includes(choice.label) ? "opacity-100" : "opacity-65"} ${
                  choice.value === 4 && "mt-[6px]"
                } h-full w-full`}
              />
            </TouchableOpacity>
          ))}
      ...
      <View className="mx-4">
        {choices.map((choice) => (
          <TouchableOpacity
            ...
            onPress={() =>
              identificador === 1 || identificador === 2 ? setSelected([choice.value]) : BotoesSelecionados(choice.value)
            }
          >
            ...
            */}
            ...
            <Text className="text-lg text-[#323232]" style={selected.includes(choice.value) && { color: "white" }}>
              {choice.label}
            </Text>
          </TouchableOpacity>
        ))}
