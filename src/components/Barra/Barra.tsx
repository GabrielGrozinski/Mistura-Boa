...
export default function Barra() {
    ...
    function redirecionarUser(telaAtual: number) {
        ...
        } else if (telas[telaAtual].proximaTela === "AdicionarAmigos") {
            return;
        } else if (telas[telaAtual].proximaTela === "Ranking") {
            return;
        } else if (telas[telaAtual].proximaTela === "GeradorReceita") {
            navigation.navigate("GeradorReceita", { usuarioAtual });
        } else if (telas[telaAtual].proximaTela === "CriarDieta") {
            return;
        } else if (telas[telaAtual].proximaTela === "DietaCriada") {
            return;
        } else if (telas[telaAtual].proximaTela === "Receita") {
            return;
        } else if (telas[telaAtual].proximaTela === "ReceitasFavoritas") {
            return;
        } else if (telas[telaAtual].proximaTela === "ReceitasCriadas") {
            return;
        } else {
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {telas.map((icone, index) => (
                    <Pressable
                        ...
                        style={barra === index ? { borderColor: "#6B91D6", borderWidth: 3, borderRadius: 15 } : undefined}
                    >
                        <Image
                            source={icone.img}
